// HKS Law OS app.js v19-CLAUDE-AI
...
Keep each recommendation under 60 words. Be direct, commercial, and specific to a small Ontario law firm. No fluff.`;
  el.innerHTML = '<div style="font-size:12px;color:var(--text2);line-height:1.8;">'
    + '<div style="margin-bottom:8px;padding:8px;background:var(--surface2);border-radius:var(--radius);border-left:3px solid var(--red);"><strong style="color:var(--red);">HIGH</strong> Add 1 staff member now.</div>'
    + '<div style="margin-bottom:8px;padding:8px;background:var(--surface2);border-radius:var(--radius);border-left:3px solid var(--amber);"><strong style="color:var(--amber);">THIS WEEK</strong> Call 3 realtors in KW.</div>'
    + '<div style="margin-bottom:8px;padding:8px;background:var(--surface2);border-radius:var(--radius);border-left:3px solid var(--amber);"><strong style="color:var(--amber);">THIS WEEK</strong> Day 1/3/7 lead follow-up.</div>'
    + '<div style="padding:8px;background:var(--surface2);border-radius:var(--radius);border-left:3px solid var(--green);"><strong style="color:var(--green);">FREE</strong> Complete Google Business Profile.</div>'
    + '</div>';
...
async function runAnalysis(){
  ...
  var prefix = 'Expert law firm growth analysis for HKS Law, Kitchener-Waterloo Ontario, practicing Real Estate, Immigration, L&T, Insurance Claims. Ontario 2026 compliance. Be specific with numbers and actionable steps.\n\n';
  window.open('https://claude.ai/new?q=' + encodeURIComponent(prefix + input), '_blank');
  el.innerHTML = '<div style="padding:20px;text-align:center;color:var(--text2);"><div style="font-size:22px;margin-bottom:10px;">🧠</div><div>Analysis opened in Claude.ai - results appear in that tab.</div></div>';
}
...
async function generateEmail(){
  var tmpl = document.getElementById('email-template').value;
  var name = document.getElementById('c-name').value || '[NAME]';
  var ctx = document.getElementById('c-context').value || '';
  if(!tmpl && !ctx){ toast('Select a template or add context'); return; }
  loadTemplate();
  var prompt = 'Write a professional email for HKS Law, a small Ontario law firm in Kitchener-Waterloo. Template: ' + tmpl + '. To: ' + name + '. Context: ' + ctx + '. Professional tone, no legal advice, LSO compliant. Under 300 words. Email body only, start with greeting.';
  window.open('https://claude.ai/new?q=' + encodeURIComponent(prompt), '_blank');
  document.getElementById('c-body').value = 'Template loaded above. Claude.ai opened in new tab - copy the generated email here.';
  toast('Template loaded. Claude.ai opened - copy result here.');
}
function saveDraft(){
...
async function loadGmailInbox(){
  ...
  try{
    ...
    if(!data.messages){ 
      var s2 = document.getElementById('gmail-setup'); if(s2) s2.style.display='none';
      var gl = document.getElementById('gmail-list'); if(gl) gl.innerHTML='<div style="padding:20px;text-align:center;color:var(--text3);">No unread messages.</div>';
      return;
    }
    var setupEl = document.getElementById('gmail-setup');
    if(setupEl) setupEl.style.display='none';
    var badgeEl = document.getElementById('b-gmail');
    if(badgeEl) badgeEl.textContent = data.messages.length;
    var countEl = document.getElementById('count-lead');
    if(countEl) countEl.textContent = data.messages.length+' emails';
...
function aiPromptCustom(p){
  document.getElementById('ai-input').value=p;
  nav('aiops');
  var prefix = 'HKS Law context: small Ontario law firm, Kitchener-Waterloo, Real Estate, Immigration, L&T, Insurance Claims, Ontario 2026 compliance.\n\n';
  window.open('https://claude.ai/new?q='+encodeURIComponent(prefix+p), '_blank');
...
function askAI(){
  ...
  var prefix = 'You are the AI Operating Partner for HKS Law, a small Ontario law firm in Kitchener-Waterloo. Practice areas: Real Estate, Immigration, Landlord and Tenant, Insurance Claims. Ontario 2026 compliance. Tag tasks as LAWYER ONLY, STAFF, or AUTOMATABLE. Be direct and commercial.\n\nQuestion: ';
  var url = 'https://claude.ai/new?q=' + encodeURIComponent(prefix + input);
  window.open(url, '_blank');
  var el = document.getElementById('ai-output');
  el.innerHTML = '<div style="padding:24px;text-align:center;color:var(--text2);"><div style="font-size:28px;margin-bottom:12px;">🤖</div><div style="font-size:14px;font-weight:500;margin-bottom:8px;">Opened in Claude.ai</div><div style="font-size:12px;color:var(--text3);">Your question was sent to Claude.ai with full HKS Law context. The answer will appear there in a new tab.</div></div>';
  log('AI query: '+input.substring(0,50),'');
  toast('Opening Claude.ai...');
}
function aiDraftContent(topic){var p='You are the AI Operating Partner for HKS Law, a small Ontario law firm in Kitchener-Waterloo. Practice areas: Real Estate, Immigration, Landlord and Tenant, Insurance Claims. Ontario 2026 compliance. Tag tasks: LAWYER ONLY, STAFF, or AUTOMATABLE. Be direct and commercial.\n\nWrite a LinkedIn post for HKS Law about: '+topic+'. Educational, under 200 words, subtle CTA, LSO compliant, no superlatives.';window.open('https://claude.ai/new?q='+encodeURIComponent(p),'_blank');toast('Content sent to Claude.ai!');}". The post should be educational (not promotional), position HKS Law as the local expert in KW, be under 200 words, include a subtle call to action, and comply with LSO advertising rules (no superlatives, no outcome promises).`;
  nav('aiops');askAI();
