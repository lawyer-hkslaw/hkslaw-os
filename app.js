// HKS Law OS app.js v10 - GMAIL-OAUTH
...
async function loadGmailInbox(){
  ...
  try{
    ...
    document.getElementById('gmail-list').innerHTML = details.map(function(msg){
      var headers = msg.payload.headers;
      var from = (headers.find(function(h){return h.name==='From';})||{value:'Unknown'}).value;
      var subject = (headers.find(function(h){return h.name==='Subject';})||{value:'(no subject)'}).value;
      var fromName = from.replace(/<[^>]+>/,'').trim().replace(/"/g,'') || from;
      var isUnread = msg.labelIds && msg.labelIds.includes('UNREAD');
      return '<div class="gmail-item '+(isUnread?'unread':'')+'" onclick="draftReply(''+msg.id+'',''+subject.replace(/'/g,'').substring(0,40)+'',''+fromName.replace(/'/g,'').substring(0,30)+'')">'
        +'<div style="display:flex;justify-content:space-between;align-items:center;">'
        +'<div class="gmail-sender">'+fromName.substring(0,30)+'</div>'
        +'</div>'
        +'<div class="gmail-subject">'+subject.substring(0,60)+'</div>'
        +'<div class="gmail-preview">Click to draft a reply with AI</div>'
        +'<div style="margin-top:8px;"><span class="tag tag-auto">'+( isUnread?'unread':'read')+'</span></div>'
        +'</div>';
    }).join('');
  }catch(e){
